...
def test_get_error_calculations(typed_metrics_df: pd.DataFrame) -> None:
    """Test that base error derivations calculate correctly and preserve types."""
    df = get_error(typed_metrics_df.copy())
...
def test_get_metrics_accuracy(typed_metrics_df: pd.DataFrame) -> None:
    """Test the final aggregate metric calculations for mathematical accuracy."""
    metrics: dict[str, float] = get_metrics(typed_metrics_df.copy())
    ...
    assert np.isclose(metrics["Percentage of Errors > 10"], expected_pct_over_10)
